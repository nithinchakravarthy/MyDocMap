...
class SignupVC: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func signupTapped(sender: UIButton) {
        ...
        var confirm_password:NSString = txtConfPass.text! as NSString
        ...
        if ( username.isEqualToString("") || password.isEqualToString("") ) {
            ...
            let urlData: NSData? = NSURLConnection.sendSynchronousRequest(request, returningResponse:&response, error:&reponseError)
            ...
            if ( urlData != nil ) {
                ...
                if (res.statusCode >= 200 && res.statusCode < 300)
                {
                    ...
                    do{
                        if var jsonData:NSDictionary = try NSJSONSerialization.JSONObjectWithData(urlData!, options:NSJSONReadingOptions.MutableContainers) as? NSDictionary {
                            print(jsonData)
                        }
                    }
                    catch let error as NSError {
